...
from util import seed_everything, load_config, prompt_infilling_batch_SoT, tok_batch_encode, stop_sequences_criteria, tok_decode, save_json
...
def step_processing(batch, prompt, model, tokenizer, padding_side, left_truncate_len, truncation, until, device, **generation_kwargs):
    # Replacing the variables with actual values/questions in prompt
        inputs = prompt_infilling_batch_SoT(batch, prompt)
...
def main(config):
    ...
    for batch in tqdm(data_loader):
        batch, inputs, res = step_processing(batch, skeleton_prompt, model, tokenizer, padding_side, left_truncate_len ,truncation, until, device, **generation_kwargs)
